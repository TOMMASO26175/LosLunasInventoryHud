...
function secondInventorySetup(invOwner, items, invTier, money) {
    setupSecondarySlots(invOwner,secondTier.label);
...
function setupSecondarySlots(owner,itype) {
    $('#inventoryTwo').html("");
    $('#other-inv-id').html("");
    ...
    for (i = 1; i <= (secondTier.slots); i++) {
        ...
        //$('#inventoryTwo').find('.slot-template').addClass('temporary');
            //startsWith("car")|| owner.startsWith("container") || owner.startsWith("car") || owner.startsWith("pd-trash")
        //InventoryLog(itype)
        if (itype.startsWith("Camion")|| itype.startsWith("container") || itype.startsWith("car") || itype.startsWith("Getta")) {
            $('#inventoryTwo').find('.slot-template').addClass('temporary');
        } else if (itype.startsWith("pv") || itype.startsWith("Magazzino")) {
            $('#inventoryTwo').find('.slot-template').addClass('storage');
        } else if (itype.startsWith("Player")) {
            $('#inventoryTwo').find('.slot-template').addClass('player');
        } else if (itype.startsWith("pd-evidence")) {
...
$(document).ready(function () {
    ...
    function GetQuantity(num,itemData) {
    ...
    $('#textinput').keypress(function(event){
        ...
        if(keycode == '13'){
            //$('#bottone').click();
            InventoryLog(quantityvalue)
            GetQuantity(quantityvalue,cursItemData);
            //cursItemData = null
        }
    });
