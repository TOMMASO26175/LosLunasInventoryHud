...
RegisterServerEvent('disc-inventoryhud:ricaricaammo')
AddEventHandler('disc-inventoryhud:ricaricaammo', function(item, count, playerSource)
    local _source = playerSource or source
    if impendingRemovals[_source] == nil then
        impendingRemovals[_source] = {}
    end
...
-- 	if count > 0 then
-- 		LS.UseItem(source, itemName)
-- 	else
-- 		xPlayer.showNotification(_U('act_imp'))
-- 	end
-- end)

-- LS.UseItem = function(source, item)
...
AddEventHandler('esx:onRemoveInventoryItem', function(source, item, count)
    ...
    applyToInventory(player.identifier, 'player', function(inventory)
        ...
        removeItemFromInventory(item, count, inventory)	--boh
        TriggerClientEvent('disc-inventoryhud:refreshInventory', source)
    end)
end)
